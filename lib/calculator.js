...
function calculate(event) {
  ...
  if (clickedButtonValue === '=') {
    ...
    if (display.value !== '') {
      ...
      if (display.value !== '' && (!display.value.includes('.') || operators.includes(display.value.charAt(display.value.length - 2) || operators.includes(display.value.charAt(display.value.length - 1))))) {
        // Add decimal point
        display.value += '.';
      }

  } else if ((operators.includes(clickedButtonValue)) && (output.value !== '')) {
